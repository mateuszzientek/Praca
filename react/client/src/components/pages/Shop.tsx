...
function Shop() {
    ...
    const {
        ...
        setSelectedSizes,
        selectedSort,
        setSelectedSort } = useContext(FilterContext);
    ...
    const sortType = ['newest', 'priceLow', 'priceHigh'];

    const { pageNumber, brand, category, price, min, max, sizes, sort } = useParams();
    ...
    const handleMaxBlur = () => {
        if (currentMax && validator.isNumeric(currentMax) && parseInt(currentMax) > 0) {
            if (currentCode === 'pl') {
    ...
    useEffect(() => {
        ...
        const fetchShoes = async () => {
            try {
                const response = await axios.get(`/shoes?page=${page}&brand=${selectedBrand}&category=${selectedCategory}&price=${selectedPrice}&min=${selectedMin}&max=${selectedMax}&sizes=${selectedSizes.join(",")}&sort=${selectedSort}`);
        ...
        if (selectedBrand || selectedCategory || selectedPrice || selectedMin || selectedMax || selectedSizes || selectedSort) {
            let url = `/shop/${page}`;
    ...
    }, [page, selectedBrand, selectedCategory, selectedPrice, selectedMin, selectedMax, selectedSizes, selectedSort, user]);
            ...
            <div className='flex justify-center   xl:px-12 2xl:px-20 bg-white dark:bg-[#292929] min-h-screen '>
                ...
                <div className='hidden lg:flex flex-col items-center w-[270px] min-w-[270px] h-auto  border-r-[2px] dark:border-white border-black/10'>
                    <div className='text-left '>
                        ...
                        <div className='flex justify-center'>
                            <button onClick={handleReset} className='px-2 py-1 border-2 border-black/60 text-black dark:text-white dark:border-white rounded-lg mt-6 hover:scale-105 transform ease-in-out duration-500 '>
                                <p>{t('shop.reset')}</p>
                            </button>
                        </div>
                ...
                <div className='flex flex-col h-full w-[100rem] pb-20'>
                    ...
                    <p className=' text-black/80 dark:text-white text-3xl px-12 py-10 text-center lg:text-left '>{selectedBrand === "All" ? t('shop.all') : selectedBrand}</p>
                    <div className='flex justify-between '>
                        ...
                        <div className='relative flex xl:mr-20 mr-12 2xl:mr-12 h-[3rem] border-[2px] border-black/10 dark:border-white dark:text-white text-black/60 rounded-md'>
                            <button onClick={() => setShowSort(!showSort)} className='flex items-center space-x-1 px-2'>
                                <p>{!selectedSort ? t(`shop.sort`) : t(`sort.${selectedSort}`)}</p>
                                {showSort ? <AiOutlineUp size={15} /> : <AiOutlineDown size={15} />}
                                ...
                                <div className='animate-sort-in absolute top-12 right-0 w-[10rem]   bg-white shadow-button mt-1 rounded z-10'>
                                    {sortType.map((sort) => (
                                        <label key={sort} className='cursor-pointer '>
                                            ...
                                            <p className='text-black/80 px-3 py-2 peer-checked:font-bold hover:bg-black/10 '>{t(`sort.${sort}`)}</p>
                                        </label>
                                    ))}
                                </div>
                            )}
                    ...
                    {loading ? (
                        ...
                        <div className='flex justify-center items-center h-[50vh]'>
                            <p className='text-lg text-black/80 dark:test-white md:text-3xl'>{t('shop.notFound')}</p>
                        </div> :
                        (
                            <>
