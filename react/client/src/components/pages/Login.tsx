...
function Login() {
  ...
  const validateData = () => {
    ...
    return (
      Object.keys(
        newErrors.email || newErrors.name || newErrors.surname || newErrors.pass
      ).length === 0
    );
  };
    ...
    <>
      ...
      <div className="bg-[#ececec] dark:bg-black/80 w-screen min-h-screen flex items-center justify-center ">
        <div className="absolute left-10 top-8 ">
          <div
            onClick={() => navigate("/")}
            className="flex items-center space-x-2 cursor-pointer hover:scale-105 transform ease-in-out duration-300"
          >
        ...
        <div className="flex flex-col lg:flex-row my-20 w-[90%] lg:w-[80%] 2xl:w-[60%] h-screen/2 shadow-button rounded-3xl animate-fade-in-long">
          ...
          <div className="flex flex-col items-center bg-white dark:bg-black/60 w-full lg:w-[50%]  xl:w-[50%] rounded-b-3xl lg:rounded-l-none lg:rounded-r-3xl">
            ...
            <form className="w-[70%]" onSubmit={handleSubmit}>
              ...
              {!isLoginSelected && errors.surname && (
              ...
              {errors.pass && (
                <p className="text-red-500 text-sm mt-2 ml-2">{errors.pass}</p>
              )}

              {errorsServer && (
                <p className="text-red-500 text-sm mt-2 ml-2">{errorsServer}</p>
              )}
              ...
              {!isLoginSelected && (
                <div className="flex justify-center items-center space-x-4 mt-6">
                  <input
                    onChange={handleEmailOffertChange}
                    type="checkbox"
                    className="w-5 h-5 checked:bg-blue-500 cursor-pointer "
                  />
                  <p className="font-lato w-[100%] text-sm text-black dark:text-white">
                    {t("login.text")}
                  </p>
                </div>
              )}
            ...
            {!isLoginSelected && (
              <p className="font-lato w-[70%] text-sm mt-6 text-black dark:text-white ">
                {t("login.text4")}{" "}
                <span className="font-bold">{t("login.text5")}</span>{" "}
                {t("login.text6")}{" "}
                <span className="font-bold">{t("login.text7")}</span> .
              </p>
            )}
            ...
            {isLoginSelected && (
              <button
                ...
                className="text-black dark:text-white font-lato text-xl mt-6 hover:scale-105 transition ease-in-out duration-300"
              >
                {t("login.forgot")}
              </button>
            )}
            ...
            <div className="flex items-center justify-center w-[60%] mt-10">
              ...
              <p className="text-base px-2 text-black dark:text-white ">
                {isLoginSelected ? t("login.text2") : t("login.text3")}{" "}
              </p>
              <div className="border-b border-black/50 dark:border-white w-[20%]"></div>
            </div>
