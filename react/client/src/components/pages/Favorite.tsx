...
function Favorite() {
        ...
        const fetchFavoriteShoes = async () => {
            try {
                if (isUserDataLoaded && !user?._id) {
                    ...
                    setLoading(true);
                    const response = await axios.get(`/getFavoriteShoes/?userId=${user?._id}`);
    ...
    const handleHeartClick = (shoe: Shoe) => {
        axios
            .post("/removeFavoriteShoe", { userId: user?._id, shoeId: shoe._id })
            .then((response) => {
        ...
        const updatedShoes = favoriteShoes.filter((prevShoe) => prevShoe._id !== shoe._id);
    ...
    return (
        <div className="bg-white dark:bg-black/80 min-h-screen">
            ...
            <div className="flex justify-center">
                <p className="text-4xl text-black/80 dark:text-white mt-16">{t("favoriteShoes.text1")}</p>
            </div>
            ...
            {loading ? (
                ...
                <div className="flex justify-center items-center mt-20">
                    <p className='text-base md:text-2xl text-black/70 dark:text-white/70'>{t("favoriteShoes.text2")}</p>
                </div>
            ) : (
                <>
